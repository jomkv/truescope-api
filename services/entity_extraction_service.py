...
class EntityExtractionService:
    def __init__(self, use_gazette=True, gazette_pkl_path="constants/gazetteer.pkl"):
        self.entity_extractor = spacy.load("data/model_adapters/ner/v1")
    ...
    def extract_entities(self, text: str) -> List[Tuple[str, str]]:
        ...
        if self.use_gazette:
            ...
            # Add selected gazetteer matches only if they do not overlap with NER spans
            for start_idx, end_idx, matched_text, keyword in matches:
                if not any(i in ner_spans for i in range(start_idx, end_idx + 1)):
                    # Check for overlap with already selected gazetteer matches
                    if not any(i in occupied for i in range(start_idx, end_idx + 1)):
                        entities.add((matched_text, "GAZ"))
                        occupied.update(range(start_idx, end_idx + 1))
